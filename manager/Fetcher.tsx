...
import { SurrealQueryManager } from './Surreal';
...
export const Fetcher = (): DataProvider => ({
    getList: (resource, params) => {
        ...
        return SurrealQueryManager(query, { query: q }).then((result) => {
            if (result[0]?.result) {
                let total = 0;
                const data =
                    result &&
    ...
    getOne: (resource, params) => {
        ...
        return SurrealQueryManager(query).then((result) => {
            if (result[0]?.result) {
                return {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
